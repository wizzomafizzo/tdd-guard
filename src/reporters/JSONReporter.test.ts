...
describe('JSONReporter', () => {
  ...
  it('uses FileStorage when no storage provided', async () => {
    const sut = setupJSONReporter({ type: 'file' })
  ...
  it('accepts Storage instance in constructor', () => {
    ...
    const reporter = new JSONReporter(storage)
...
function setupJSONReporter(options?: { type: 'file' | 'memory' }) {
  ...
  if (options?.type === 'file') {
    testDir = mkdtempSync(join(tmpdir(), 'json-reporter-test-'))
  ...
  const getPassedTests = async () => {
    ...
    return tests.filter((t: { state: string }) => t.state === 'passed')
  }

  const getFailedTests = async () => {
    const tests = await getTests()
    return tests.filter((t: { state: string }) => t.state === 'failed')
  }
